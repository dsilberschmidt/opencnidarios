...
Pendiente anotado (no bloqueante): verificación cualitativa de `_compress()` con distintos historiales de prueba (variando extensión y homogeneidad) para confirmar que ambos proveedores comprimen de forma comparable con el mismo prompt. El primer run mixto real (Claude + OpenAI conviviendo en la misma arena, 30 ticks) SÍ se corrió más tarde en esta misma sesión — ver la sección "organism resume/transplant from snapshots" más abajo, que reutiliza esos mismos dos organismos (`77611abd`, `7fa90c0a`).
